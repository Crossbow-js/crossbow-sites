...
module.exports = function (cache, logger, config) {
    return getFile.bind(null, cache, config);
};
 ...
 * @param cache
 */
function getFile(cache, config, filePath, transform, allowEmpty) {
    ...
    if (content = cache.findAny(filePath)) {
        logger.debug("{green:Cache access} for: %s", filePath);
        return content.content || content;
    } else {
        logger.debug("Not found in cache: %s", filePath);
    }
     ...
     */
    try {
        logger.debug("{yellow:File System access} for: %s", filePath);
