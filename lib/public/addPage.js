    ...
    "frozenTransforms": [
        {
            when: "before freeze",
            fn: function (opts) {
                //var item  = createPage({type: "page", key: "post-list.html", content: "{{#page.related}}::{{this.url}}::{{/page.related}}"}, opts.compiler);
                //var posts = opts.compiler.cache.byType('post');
                //item = item.set('related', posts);
                //opts.compiler.cache.add(item);
            }
        }
    ],
    "types": {
