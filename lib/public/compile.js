...
module.exports = function compile(opts) {
    ...
    function afterCompile(err, out) {
        ...
        var defaultItemLayout = compiler.config.getIn(["defaultLayouts", "type:" + compiler.item.get("type")]);
         ...
         */
        if (!defaultLayout && !defaultItemLayout && !itemlayout) {
            return end(content);
        }
        ...
        /**
         * If a layout was specified in the item front-matter, always use that first
         */
        if (itemlayout) {
            selectedLayout = itemlayout;
        } else {
            if (defaultItemLayout) {
                selectedLayout = defaultItemLayout;
            }
        }
