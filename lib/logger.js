/**
 * tfunk for terminal colours
 * @type {addLayout|exports}
 */
var tfunk    = require("tfunk");

var compiler = new tfunk.Compiler({
    prefix: "[{magenta:CoderBlog}] ",
    custom: {
        "error": "chalk.bgRed.white",
        "warn": "chalk.red"
    }
});

var logLevel = "warn";

var debugPrefix = ":{cyan:DEBUG}] - ";
var infoPrefix  = ":{cyan:INFO}] - ";
var warnPrefix  = ":{red:WARNING}] - ";

//
module.exports = function (emitter) {

    emitter.on("log", function (data) {
        log(data.type, data.msg);
    });

    return {
        log: log,
        setLogLevel: setLogLevel,
        setName: setName
    };
};

/**
 * @param level
 * @param msg
 * @returns {*}
 */
function log (level, msg){

    var args = Array.prototype.slice.call(arguments);
    args = args.slice(2);
    msg = tfunk(msg);

    if (level === "warn") {
        args.unshift(warnPrefix + msg);
        return console.log.apply(console, args);
    }

    if (level === "debug" && logLevel === "debug") {
        args.unshift(debugPrefix + msg);
        return console.log.apply(console, args);
    }

    if (level === "info") {
        args.unshift(infoPrefix + msg);
        return console.log.apply(console, args);
    }
}
/**
 * @param level
 */
function setLogLevel(level) {
    logLevel = level;
}

/**
 * @param name
 */
function setName(name) {

    var prefix = "[" + name;

    debugPrefix = tfunk(prefix + ":{cyan:DEBUG}] - ");
    infoPrefix  = tfunk(prefix + ":{cyan:INFO}] - ");
    warnPrefix  = tfunk(prefix + ":{red:WARNING}] - ");
}