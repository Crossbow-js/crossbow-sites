...
module.exports = function includeHelper (cbUtils) {
    ...
    return function () {
        ...
        if (!utils.verifyParams(options.hash, ["src"])) {
            ...
            err._crossbow.message = "You must provide a `src` parameter to the include helper";
            err._type     = "include:src";
            throw err;
        }
        ...
        try {
            ...
            var processedParams = utils.processParams(options.hash, options.data.root, cbUtils);

            return new Handlebars.SafeString(errors["file:notfound"](processedParams.src));
        }
