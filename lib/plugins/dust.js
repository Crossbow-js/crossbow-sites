...
module.exports = function (file, emitter) {
    ...
    var helpers  = require("./dust-helpers")(file, emitter, dust);
        ...
        "dataTransforms": {
            ...
            "helpers": {
                ...
                fn: function (item, data, config) {
                    ...
                    dust.helpers["save"]        = helpers["save"](data);
                    dust.helpers["highlight"]   = helpers["highlight"](data);
                    dust.helpers["hl"]          = helpers["highlight"](data);
                    return data;
                }
            }
        }
 ...
 * @param getFile
 */
function getSnippetInclude(filePath, data, chunk, params, file) {

    var fileContents = file.getFile(filePath, null, false);
    ...
    if (!fileContents) {
        ...
        return chunk.map(function (chunk) {
            return renderTemplate(utils.wrapSnippet(fileContents, lang), data, function (err, out) {
                if (err) {
                    chunk.end("");
                } else {
                    chunk.end(out);
