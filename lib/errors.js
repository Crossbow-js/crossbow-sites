...
var fails    = {
    ...
    "lodash:string:params": function (data) {
        var crossbow = objPath.get(data, "error._crossbow", {});
        if (crossbow.message) {
            var str = crossbow.message;
            str += "\n";
            ...
            str += "File:     {yellow:" + crossbow.file + "}\n";
            str += "\n";
            str += "Example 1:  {yellow:\\{\\{$_ 'camelCase' 'Foo Bar'\\}\\}}\n";
            str += "Output:     {green:fooBar\n";
            return [str];
        }
    }
};
...
var inline = {
    "imgs:none-found" : function (dir) {
        return wrapper.replace("%s", "No images found in " + dir);
    },
    "dir:notfound" : function (dir) {
        return wrapper.replace("%s", "Directory not found: " + dir);
    },
