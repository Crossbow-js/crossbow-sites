...
module.exports.tasks = {
    ...
    flattenBeforeTransforms: function (compiler, done) {
        ...
        compiler.template.render(compiler.item.get("compiled"), compiler.data.toJS(), function (err, out) {
...
function runDataTransforms (opts) {
    ...
    require("lodash").each(transforms, function (plugin) {
        if (plugin.when === opts.scope) {
            opts.compiler.globalData = plugin.fn(opts, opts.compiler);
        }
    });
...
function runContentTransforms(opts) {
    ...
    require("lodash").each(transforms, function (plugin) {
        if (plugin.when === opts.scope) {
            opts.item.content = plugin.fn(opts, opts.compiler);
        }
    });
