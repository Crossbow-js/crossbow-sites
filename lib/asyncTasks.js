...
function flattenBeforeTransforms (compiler, item, done) {
    ...
    // Promote 'site' vars to top-level access
    compiler.globalData.site = item.data.site;
...
function buildLayouts (compiler, item, done) {
     ...
     */
    if (!item.front.layout && !layoutPath) {
        return done();
    }

    if (item.front.layout) {
        layoutPath = item.front.layout;
    }
     ...
     */
    layouts(compiler, layoutPath, item, function (err, out) {
        done(null, {
            item: {
