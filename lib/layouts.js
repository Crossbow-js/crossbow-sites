...
function addLayout(compiler, layout, item, cb) {
    ...
    if (layoutFile && yaml.hasFrontMatter(layoutFile.content)) {
        ...
        return compiler.hb.renderTemplate(_data.content, data, function (err, out) {
    ...
    cb(null, {
        content: compiler.hb.renderTemplate(layoutFile.content, data)
    });
}
