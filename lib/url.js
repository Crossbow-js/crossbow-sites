...
function makePostUrl(key, config, categories) {
    ...
    config = config || {};
    var split      = extractDateFromKey(getTempUrl(key, config.cwd));
    var formatUrl  = getUrlFormatter(config.prettyUrls);
    
    if (hasSubDirs(key)) {
        return formatUrl(
            makeSubDirUrl(config.postUrlFormat, split.date, split.url, categories),
            split.date
        );
    }
...
function makePageUrl(key, config) {
    ...
    if (_.isString(config.cwd)) {
        key = key.replace(config.cwd, "");
    }

    var filePath = stripExtension(key);
    var url      = stripExtension(key);

    if (hasIndex(key) && config.prettyUrls) {
    ...
    return getUrlFormatter(config.prettyUrls)(
